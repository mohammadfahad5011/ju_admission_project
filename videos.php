    ...
    if (isset($video_news_data->items)) {
        foreach ($video_news_data->items as $key => $news) {
          $news_img = '/images/Default-Image.png';
            if (!empty($news->alternative) && !empty($news->alternative->home) && !empty($news->alternative->home->default)) {
            ...
            } elseif (!empty($news->{"hero-image-s3-key"})) {
                $news_img = 'https://images.prothomalo.com/' . $news->{"hero-image-s3-key"};
            }
    ?>
            <div class="card">
              <a target="_blank" href="<?php echo htmlspecialchars($news->url); ?>">
                <img src="<?php echo htmlspecialchars($news_img); ?>" alt="Image" />
                <h2><?php echo htmlspecialchars($news->headline); ?></h2>
                <h6>আরও পড়ুন</h6>
              </a>
            </div>
    <?php
